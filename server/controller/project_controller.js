module.exports = {
    ...
    getSingle: (req, res) => {
        const db = req.app.get('db') 
        db.get_single_project([req.params.project_id]).then(proj=> res.status(200).send(proj)).catch(error=>{console.error(error);res.status(500).send(err)})
    },
    getSkillStack: (req, res) => {
        const db = req.app.get('db') 
        db.get_skillstack([req.params.project_id]).then(skillstack=> res.status(200).send(skillstack)).catch(error=>{console.error(error);res.status(500).send(err)})
    },
    getProjCons: (req, res) => {
        const db = req.app.get('db') 
